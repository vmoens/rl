...
class JumanjiWrapper(GymLikeEnv):
    ...
    def _encode_state(self, state):
        ...
        _state = _jumanji_to_torchrl_data_transform(state, device=self.device)
        return TensorDict(_state, batch_size=self.batch_size, device=self.device)

    def _decode_state(self, tensordict):
        state = tensordict["state"]
        _state_container = _map_to_state(state, self._state_container)
        print(_state_container, self._state_container)
        return _state_container


def _map_to_state(tensordict, state):
    d = {}
    for key, item in tensordict.items():
        if isinstance(item, TensorDictBase):
            d[key] = _map_to_state(item, getattr(state, key))
        else:
            attr = getattr(state, key)
            dtype = attr.dtype
            shape = attr.shape
            d[key] = np.asarray(item.numpy(), dtype).reshape(shape)
            print(getattr(state, key).shape, d[key].shape)
    return type(state)(**d)
