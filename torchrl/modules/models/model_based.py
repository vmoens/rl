...
class DreamerV3ImageDecoder(nn.Module):
    ...
    ``0.5`` so that the prediction lives in the same ``[0, 1]`` range as an
    image divided by ``255``.
    ...
    Args:
        ...
        kernel_size (int, optional): Transposed convolution kernel size.
            Defaults to 5.
    ...
    def forward(
        ...
        *,
        ...
        if not action.is_floating_point():
            # One-hot action specs sample boolean actions.
