...
class ValueEstimatorBase(TensorDictModuleBase):
    ...
    def _call_value_net_compact(
        ...
        in-key from ``("final", k)`` when the collector provides it,
        falling back to a duplicate of ``root[T-1]`` otherwise (i.e.
        ``obs_T* ≈ obs_{T-1}``). The duplication is intentional for
        recurrent value nets: feeding the RNN one extra step with the
        duplicated obs advances the hidden state, so ``V(next_obs[T-1])``
        is approximated by ``f(obs_{T-1}, h_T)`` — not by a scalar copy
        of ``V_T = f(obs_{T-1}, h_{T-1})``. The two coincide for
        ...
        Shape and code path are constant within a training run (no
        ``("final", ...)``-conditional branching, no Python branches on
        tensor values, no ``.item()`` syncs), so ``torch.compile``
        specializes once and stays specialized.
        """
        ...
        # Per-key boundary overrides from ``("final", k)`` when the
        # collector provides them. Keys without a ``("final", k)`` entry
        # keep the duplicated ``root[T-1]`` value below.
        final_root = data.get("final", default=None)
        final_values: dict = {}
