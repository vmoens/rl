...
class DreamerV3ImageEncoderConfig(NetworkConfig):
    """A class to configure a DreamerV3 convolutional image encoder.
...
class DreamerV3ImageDecoderConfig(NetworkConfig):
    """A class to configure a DreamerV3 transposed-convolution image decoder.
    ...
    in_features: int = MISSING
    image_shape: list[int] = field(default_factory=lambda: [3, 64, 64])
    depth: int = 64
    mults: list[int] = field(default_factory=lambda: [2, 3, 4, 4])
    kernel_size: int = 5
    num_blocks: int = 8
